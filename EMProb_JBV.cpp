...
#include "include/EMProblem_JBV.hpp"
...
int main(int argc, char *argv[])
{
  ...
  if (verbose) args.PrintOptions(cout);
   ...
   feSpaces.push_back(new ParFiniteElementSpace(pmesh, B_coll,  1));
   ...
   vector<ParGridFunction*> JBV;
   vector<string> VarNames;
 
   for(int I=0; I<feSpaces.size(); I++){
     JBV.push_back(new ParGridFunction);
     JBV[I]->MakeRef(feSpaces[I], x_vec.GetBlock(I), 0);
     JBV[I]->Distribute(&(tx_vec.GetBlock(I)));
   }
   VarNames.push_back("J_RT_Field");
   VarNames.push_back("B_ND_Field");
   VarNames.push_back("V_H1_Potential");


   tx_vec = 0.0;
   tb_vec = 0.0;
   tx_vec.GetBlock(2) = 0.1;
   ...
   JBV[0]->ProjectBdrCoefficient(ZeroV,*BDR_markers[0]);

   Array<int> V_BDRs_tmp(nTags);
   V_BDRs_tmp = 0;
   if(nTags>1) V_BDRs_tmp[1]=1;
   if(nTags>2) V_BDRs_tmp[2]=1;
   JBV[2]->ProjectBdrCoefficient(ThreeS,V_BDRs_tmp);
   V_BDRs_tmp = 0;
   V_BDRs_tmp[0]=1;
   JBV[2]->ProjectBdrCoefficient(ZeroS,V_BDRs_tmp);
   ...
   //
   //
   for(int I=0; I<JBV.size(); I++) JBV[I]->Distribute(&(tx_vec.GetBlock(I)));
   ParaViewVisualise("EMProblem3", JBV, VarNames, order, pmesh, 0.0);
