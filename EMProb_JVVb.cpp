...
#include "include/EMProblem_JVVb.hpp"
...
int main(int argc, char *argv[])
{
  ...
  if (verbose) args.PrintOptions(cout);
   ...
   FiniteElementCollection *Vb_coll(new L2_FECollection(order+1, dim));
   ...
   vector<ParGridFunction*> JVVb;
   vector<string> VarNames;
 
   for(int I=0; I<feSpaces.size(); I++){
     JVVb.push_back(new ParGridFunction);
     JVVb[I]->MakeRef(feSpaces[I], x_vec.GetBlock(I), 0);
     JVVb[I]->Distribute(&(tx_vec.GetBlock(I)));
   }
   VarNames.push_back("J_RT_Field");
   VarNames.push_back("V_H1_Potential");
   VarNames.push_back("V_L2_Potential");
   ...
   Array<int> J_BDRs(nTags), V_BDRs(nTags), V_BDRs_tmp(nTags), empty_tag;
   ...
   if(dbcv.Size() != dbcs.Size()) cerr << "You Fool you haven't put the BC's in properly" << endl;
   if(dbcv.Size() != dbcs.Size()) return 0;
   ...
   for(unsigned I=0; I<dbcs.Size(); I++){
     ...
     JVVb[0]->ProjectBdrCoefficient(UDC,V_BDRs_tmp);
   }
   ...
   for(unsigned J=0; J<nbcs.Size(); J++){
     ...
     JVVb[1]->ProjectBdrCoefficient(UNV,V_BDRs_tmp);
   }
   for(int I=0; I<JVVb.size(); I++) JVVb[I]->Update();


   BDR_markers[0] = &J_BDRs;
   BDR_markers[1] = &V_BDRs;
   BDR_markers[2] = &empty_tag;
   ...
   applyDirchValues(*JVVb[0], tb_vec.GetBlock(0), ess_Jtdof);
   applyDirchValues(*JVVb[1], tb_vec.GetBlock(1), ess_Vtdof);
   applyDirchValues(*JVVb[0], tx_vec.GetBlock(0), ess_Jtdof);
   applyDirchValues(*JVVb[1], tx_vec.GetBlock(1), ess_Vtdof);
   applyDirchValues(*JVVb[0], x_vec.GetBlock(0),  ess_Jtdof);
   applyDirchValues(*JVVb[1], x_vec.GetBlock(1),  ess_Vtdof);
 ...
 //solver->SetPreconditioner(sampleProb.Precon());
   ...
   //
   //
   for(int I=0; I<JVVb.size(); I++) JVVb[I]->Distribute(&(tx_vec.GetBlock(I)));
   ParaViewVisualise("EMProblem2", JVVb, VarNames, order, pmesh, 0.0);
