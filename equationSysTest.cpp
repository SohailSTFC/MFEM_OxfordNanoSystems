...
#include "include/StokesProblemFacadeClass.hpp"
...
//   FiniteElementCollection *RT_coll (new RT_FECollection(order, dim));
   ...
   StokesProbFacade sampleProb(RT_coll,H1_coll,dim);
   equationSystem<StokesProbFacade>  sampleSys(sampleProb, mt, dim);

   //Set the solver and preconditioner
 // demoProb.BuildPreconditioner();
   sampleSys.Set_Solver(verbose);

   //Solve the equations
   sampleSys.Solve(verbose);

   //Visualise the results using ParaView
   double time = 0.0;
   sampleSys.SetFields();
   ParaViewVisualise("StokesSampleProb",sampleSys.Fields, sampleSys.FieldNames, order, pmesh, time);
