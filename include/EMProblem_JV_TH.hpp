#ifndef EMPROBLEM_JV_TH_HPP
#define EMPROBLEM_JV_TH_HPP
                     ...
                     mu(1.0), sigma(1.0), one(1.0)
{
  ...
  JJForm->AddDomainIntegrator( new VectorMassIntegrator(one) );
  JJForm->Assemble();
  JJForm->FormSystemMatrix(ess_Jtdof, OpJJ);

  //Mixed Bilinear Forms
  JVForm->AddDomainIntegrator( new GradientIntegrator(sigma));
  JVForm->Assemble();
